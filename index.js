...
function verifierSaisie(champ) {
    ...
    if (valeur === "" && champ.hasAttribute("required")) {
        console.error("❌ [Accueil] Champ '" + nom + "' vide et obligatoire.");
        return false;
    }
    if (champ.type === "email" && valeur !== "") {
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(valeur)) {
            console.error("❌ [Accueil] Email invalide : " + valeur);
            return false;
        }
    }
    return true;
...
for (var i = 0; i < forms.length; i++) {
    forms[i].addEventListener("submit", function (e) {
        ...
        if (!ok) console.error("🚫 [Accueil] Soumission bloquée - erreurs de saisie.");
    });
}
